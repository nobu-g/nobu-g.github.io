...
const Resume = ({ data }: ResumeProps) => {
  if (!data) {
    return <section id="resume" className={style.resume}></section>
  }

  const education = data.education.map((edu) => (
    <div key={edu.school}>
      <h3 className={style.h3}>{edu.school}</h3>
      <p className={style.info}>
        {edu.degree} <span>&bull;</span>
        <em className={style.date}>{edu.graduated}</em>
      </p>
      <p>{edu.description}</p>
    </div>
  ))

  const publications = Object.entries(data.publications).map(([kind, items]) => (
    <div key={kind}>
      <h3>{kind}</h3>
      <ul>
        {items.map((item) => (
          <li key={item.title}>
            ...
            <p>
              ...
              {item.misc}
              <br />
              {Object.entries(item.resource).map(([resourceKind, url]) => (
                <span key={resourceKind}>
                  [{url ? <a href={url}>{resourceKind}</a> : resourceKind}]
                </span>
              ))}
              {item.award && (
                <>
  ...
  const experiences = Object.entries(data.experience).map(([kind, items]) => (
    <div key={kind}>
      <h3>{kind}</h3>
      <ul>
        {items.map((item) => (
  ...
  const honors = data.honors.map((honor) => (
    <div key={honor.name}>
      <h3>{honor.name}</h3>
      <p className={style.info}>
        <em className={style.date}>{honor.years}</em>
        <br />
        {Object.entries(honor.resource).map(([resourceKind, url]) => (
          <span key={resourceKind}>
            [{url ? <a href={url}>{resourceKind}</a> : resourceKind}]
          </span>
        ))}
